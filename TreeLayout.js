		...
		return declare(null, {
			...
			createMenuHtml: function (data, li_className, a_className, ul) {
				...
				if (data.href || data.href === '') {
					...
					aTag = dc.place(dc.create('a', {href: 'javascript:void(0);', class: a_className, dialog: data.show}), li);
				}
								...
								for (var k = 0; k < menuLevel.length; k++) {
									if (menuItem[j] === menuLevel[k].name){
										if (menuLevel[k].href) {
											...
											var aTag = dc.place(dc.create('a', {href: 'javascript:void(0);', skip: menuLevel[k].href}), dd);
											dc.place(dc.create('i', {class: 'icon_' + menuLevel[k].id}), aTag);
											dc.place(dc.create('span', {innerHTML: menuLevel[k].name}), aTag);
										} else if (menuLevel[k].item) {
											var dd = dc.place(dc.create('dd'), dl);
											var aTag = dc.place(dc.create('a', { href: 'javascript:void(0);', show: menuLevel[k].id + 'Panel'}), dd);
											dc.place(dc.create('i', {class: 'icon_' + menuLevel[k].id}), aTag);
											dc.place(dc.create('span', {innerHTML: menuLevel[k].name}), aTag);
											...
											for (var ki = 0; ki < menuLevel3.length; ki++) {
												if (menuLevel3[ki].href) {
													...
													var aTag = dc.place(dc.create('a', {href: 'javascript:void(0);', skip: menuLevel3[ki].href}), li);
													dc.place(dc.create('i', {class: 'icon_' + menuLevel3[ki].id}), aTag);
